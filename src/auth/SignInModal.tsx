...
export default function SignInModal({ open, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (w.google?.accounts?.id) {
        ...
        // Render the actual Google button into hidden container
        const hiddenBtn = document.getElementById("google-signin-hidden");
        if (hiddenBtn) {
          w.google.accounts.id.renderButton(hiddenBtn, {
            theme: "outline",
            size: "large",
            width: 400,
          });
  ...
  // ---------------------------------------------------------------------------
  // Google Sign-In (trigger the hidden button)
  // ---------------------------------------------------------------------------
  function startGoogle() {
    const hiddenBtn = document.getElementById("google-signin-hidden");
    const actualButton = hiddenBtn?.querySelector("div[role='button']") as HTMLElement;
    if (actualButton) {
      actualButton.click();
    } else {
  ...
  // ---------------------------------------------------------------------------
  // OAuth callback listener (GitHub & Microsoft)
  // ---------------------------------------------------------------------------
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="relative w-full max-w-md rounded-2xl bg-gray-900 p-8 shadow-xl border border-gray-700">
        ...
        {info && (
          <div className="mb-4 rounded-lg bg-cyan-500/20 border border-cyan-500/40 p-3 text-sm text-cyan-200">
            {info}
          </div>
        )}

        {/* OAuth buttons — only on login mode */}
        {mode === "login" && (
          <>
            <div className="space-y-3 mb-6">
          {/* Hidden Google button (actual OAuth button) */}
          <div id="google-signin-hidden" className="hidden" />
          
          {/* Google - Custom styled trigger button */}
          <button
            onClick={startGoogle}
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 rounded-lg border border-gray-600 bg-gray-800 px-4 py-3 text-sm font-medium text-white hover:bg-gray-700 transition disabled:opacity-50"
          >
